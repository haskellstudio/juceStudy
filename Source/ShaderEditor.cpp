		...
		if (tabShader)
		{
			...
			if (vertexEditorComp)
			{
				if (  comboBox->getSelectedItemIndex() == 0)
				{
					...
					{
						vertexEditorComp->getDocument().replaceAllContent(g_shaderData._shaderPreset[comboBox->getSelectedItemIndex()]->vertexShader.replace(";", ";\n"));
					}
					//if (!fragmentEditorComp->getDocument().getLine(0).contains("/1"))
					{
						fragmentEditorComp->getDocument().replaceAllContent(g_shaderData._shaderPreset[comboBox->getSelectedItemIndex()]->fragmentShader.replace(";", ";\n"));
					}
				}
